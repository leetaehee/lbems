...
/**
 * Class AddElectricAllNedOb 전기 전체 전력 조회
 */
class AddElectricAllNedOb extends Command
{
    /** @var string|2004 $complexCodePk 단지번호 */
    private string $complexCodePk = '2004';
    ...
    private function insertElectricElectricMeterData(string $complexCodePk, string $type, int $option, array $data)
    {
        ...
        switch ($type) {
            case '전체 전력':
                $sensorNo = '2004_1_2_3_F';
                $val = $fcData['1층 계단실'] + $fcData['신재생 소비량'];
                $valDate = date('YmdHis');
                $dong = 'A';
                $ho = '000';
                break;
            ...
            case '1층 전체 전력':
                $sensorNo = '2004_1F';
                $val = $fcData['1층 계단실'] + $fcData['1층 사무실'];
                $valDate = date('YmdHis');
                $dong = 'A';
                $ho = '191';
                break;
