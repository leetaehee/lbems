...
/**
 * AddElectricAllDado 다도해 국립공원 전기 층,룸 사용량 조회
 */
class AddElectricAllDado extends Command
{
    /** @var string|2005 $complexCodePk 단지번호 */
    private string $complexCodePk = '2005';
    ...
    public function execute(array $params) :? bool
    {
        ...
        $this->insertElectricElectricMeterData($complexCodePk, '지하1층 전체 전력', $option, $electricData['지하 1층 전체 전력']);
        ...
        $this->insertElectricElectricMeterData($complexCodePk, '옥상 전체 전력', $option, $electricData['옥탑 전체 전력']);
    ...
    private function insertElectricElectricMeterData(string $complexCodePk, string $type, int $option, array $data) : void
    {
        ...
        switch ($type)
        {
            ...
            case '한전 전체 전력':
                $sensorNo = '2005_ALL';
                //$val = $fcData['1층 전체 전력1'] - ($fcData['신재생1'] + $fcData['신재생2']);
                $val = $fcData['1층 전체 전력1'] + $fcData['신재생1'] + $fcData['신재생2'];
                $valDate = date('YmdHis');
                $dong = 'A';
                $ho = '190';
            ...
            case '지하1층 전체 전력' :
                $sensorNo = '2005_B1';
                $val = $fcData['지하 1층 전체 전력1'] + $fcData['지하 1층 전체 전력2'];
                $valDate = date('YmdHis');
                $dong = 'A';
                $ho = '194';
                break;
            case '1층 전체 전력':
                $sensorNo = '2005_1F';
                //$val = $fcData['1층 전체 전력1'] - ($fcData['옥탑 전체 전력1'] + $fcData['지하 1층 전체 전력1'] + $fcData['지하 1층 전체 전력2'] + $fcData['2층 전체 전력1'] + $fcData['운송']);
                $val = $fcData['전기1'] + $fcData['전등1'] + $fcData['전등2'] + $fcData['전등3'] + $fcData['전등4'] + $fcData['냉방1'] + $fcData['급탕1'] + $fcData['환기1'] + $fcData['환기2'];
                $valDate = date('YmdHis');
                $dong = 'A';
                $ho = '191';
                break;
            ...
            case '옥상 전체 전력':
                $sensorNo = '2005_PH';
                $val = $fcData['옥탑 전체 전력1'] + $fcData['옥탑 전체 전력2'];
                $valDate = date('YmdHis');
                $dong = 'A';
                $ho = '193';
                break;
