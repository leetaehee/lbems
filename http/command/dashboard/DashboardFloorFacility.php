...
/**
 * Class DashboardFloorFacility
 */
class DashboardFloorFacility extends Command
{
    /** @var Usage|null usageDataModule 사용량 요금 모듈 */
    private ?Usage $usage = null;

    /** @var Indication|null $indication 자립률 모듈 */
    ...
    /**
     * DashboardFloorFacility constructor.
     */
    ...
    /**
     * DashboardFloorFacility Destructor.
     */
    ...
    public function execute(array $params) :? bool
    {
        ...
        if (is_bool($predictLoading) === true && $predictLoading === true) {
            ...
            // 설비별 사용현황 및 효율 조회
            $facilityData = $this->getFacilityData($complexCodePk, $dateType, $dongType, $floorType, $roomType);
            // 예측 데이터 조회
            $predictData = $this->getPredictUsedData($complexCodePk, $date, $predictDateType, $dongType, $floorType, $roomType);
            // 단위 면적당 사용현황 조회
            ...
            // 건물 위치 센서 조회
            $sensor = $this->usage->getBuildingLocationSensor($complexCodePk, $dongType, $floorType, $roomType);

            if (is_null($usageData) === false && is_null($facilityData) === false) {
                // 용도별 분포도에 항목추가
                $electricBuildingUsed = $this->getBuildingElectricUsed($complexCodePk, $dateType, $dongType, $floorType, $roomType, $sensor);
     ...
     * @throws \Exception
     */
    private function getFacilityData(string $complexCodePk, int $dateType, string $dong, string $floor, string $room) :? array
    {
        $fcData = [];
        $addOptions = [];
    ...
    private function addUsageItem(string $addItemKey, int $electricUsed, array $originItems, array $addItems) : array
    {
        ...
        if ($addItemKey === 'power_train'
            ...
            $fcData['distribution']['power_train'] = $indications->getUseDistribution($usageSum, $electricUsed);
            $fcData['usage']['power_train'] = $usageSum;
        }
