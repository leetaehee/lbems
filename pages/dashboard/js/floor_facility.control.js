...
function createControl()
{
    let control = {
        ...
        updateUsageInfo: function()
        {
            ...
            let pieUsages = Array.from({length:arraySize}, () => 0);

            $.each(usages, function(key, values) {
                // 사용량
                pieUsages[arrayIndex] = values;
                arrayIndex++;
            });

            arrayIndex = 1; // 분포도 생성하기 위해 1로 초기화
            ...
            chartPieUse.update(LABEL_USAGES, COLOR_USAGES, pieUsages, pieDistributions);
        },
        ...
        {
            let self = control;
            let period = self.selectedEnergyData;
        ...
        onCheckboxFloorClicked: function($this)
        {
            ...
            labelFloorName = (self.selectedFloorType === 'all') ? '전체' : labelFloorName;
        ...
        requestFloorTable: function()
        {
            ...
            const FloorInfoKeyLength = ELECTRIC_FLOOR_VALUES.length;
            ...
            $.each(FLOOR_INFOS, function(floor, floorName){
                ...
                if (jQuery.inArray(floorName, ELECTRIC_FLOOR_VALUES) === -1) {
                    return true
                }
